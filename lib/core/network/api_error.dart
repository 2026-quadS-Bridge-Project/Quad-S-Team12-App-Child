...
/// Standard error payload shape per `docs/api-contract.md`:
...
/// Parses [response.data] and returns the contract-defined `error` map, or
/// `null` if the body is not a Map or has no `error` field.
...
///
/// The [Failure.cause] field is set to the server `error.code` (when
/// available) or the HTTP status string, so call sites that need to branch
/// on a specific code can do so without re-parsing the response.
Failure<T> failureFromDioException<T>(DioException e) {
  // 1. Transport-level errors: connection drops, timeouts.
  switch (e.type) {
  ...
  // 2. Contract-shaped error body wins over status-code defaults.
