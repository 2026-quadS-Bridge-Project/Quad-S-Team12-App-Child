...
/// Standard inline top bar used across mypage, password_change, mission info,
/// time settings, and similar secondary screens.
///
/// Layout (Stack):
/// - Left: 24x24 back chevron at the same visual inset as notifications
...
class BridgeAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  /// Visual edge inset so the 24px back icon aligns with notifications.
  /// The hit target is 44px wide (Material / HIG minimum) and centred on
  /// the icon, so the wrapping box is positioned at
  /// `_edgeInset - (_backHitSize - _backIconSize) / 2` = 10 from the edge.
    ...
    return SizedBox(
      ...
      child: Stack(
        children: [
          ...
          // Leading back button. Positioned so the 44px hit box centres the
          // 24px icon at the notification-header visual inset.
          if (showBack)
            Positioned(
              left: _backHitOffset,
              top: 0,
